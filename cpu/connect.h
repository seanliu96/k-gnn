...
template <>
struct Connect<2> {
    static Tensor local(Tensor row, Tensor col, map<vector<int64_t>, int64_t> set_to_id) {
    ...
    static Tensor malkin(Tensor row, Tensor col, map<vector<int64_t>, int64_t> set_to_id) {
        auto row_data = row.data<int64_t>(), col_data = col.data<int64_t>();
        vector<int64_t> rows, cols;
