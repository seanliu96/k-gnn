...
template <>
struct Assignment<2> {
    static AssignmentType unconnected(Tensor row, Tensor col, Tensor x, int64_t num_nodes) {
        ...
        ITERATE_NODES(0, u, num_nodes, {
            ITERATE_NODES(u + 1, v, num_nodes, {
                ...
                iso_type.push_back(Isomorphism<2, false>::type({u, v}, row, col, x, num_labels));
                i++;
            });
        });
    ...
    static AssignmentType connected(Tensor row, Tensor col, Tensor x, int64_t num_nodes) {
        ...
        ITERATE_NODES(0, u, num_nodes, {
            ITERATE_NEIGHBORS(u, v, row_data, col_data, {
                if (u >= v) continue;
                set_to_id.insert({{u, v}, i});
                iso_type.push_back(Isomorphism<2, true>::type({u, v}, row, col, x, num_labels));
                i++;
            });
        });
        ...
        ITERATE_NODES(0, u, num_nodes, {
            ITERATE_NODES(u + 1, v, num_nodes, {
                ITERATE_NODES(v + 1, w, num_nodes, {
                    ...
                    iso_type.push_back(Isomorphism<3, false>::type({u, v, w}, row, col, x, num_labels));
                    i++;
                });
            });
        });
            ...
            ITERATE_NEIGHBORS(u, v, row_data, col_data, {
                ITERATE_NEIGHBORS(v, w, row_data, col_data, {
                    ...
                    if (iter == set_to_id.end()) {
                        ...
                        iso_type.push_back(Isomorphism<3, true>::type(set, row, col, x, num_labels));
                        i++;
                    }
                });
            });
