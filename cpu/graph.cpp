...
Tensor assignment_2to3(Tensor index, int64_t num_nodes) {
    ...
    map<vector<int64_t>, int64_t> set2_to_id = get<0>(Assignment<2>::unconnected(row, col, one, num_nodes));
    map<vector<int64_t>, int64_t> set3_to_id = get<0>(Assignment<3>::connected(row, col, one, num_nodes));
